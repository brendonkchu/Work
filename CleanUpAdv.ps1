...
$DaysToDelete = 7
$LogDate = get-date -format "MM-d-yy-HH-mm"
...
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete)) } |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The Contents of Temp have been removed successfully!

## Delets all files and folders in VMware VDM Logs folder. 
Get-ChildItem "C:\ProgramData\VMware\VDM\logs\*" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of VMware VDM Logs folder have been removed successfully!

## Delets all files and folders in VMware VDM Dumps folder. 
Get-ChildItem "C:\ProgramData\VMWare\VDM\Dumps\*" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of VMware VDM Dumps folder have been removed successfully!

## Delets all files and folders in user's Adobe Acrobat DC ConnectorIcons folder. 
Get-ChildItem "C:\users\*\AppData\LocalLow\Adobe\Acrobat\DC\ConnectorIcons\*" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of Adobe Acrobat DC ConnectorIcons have been removed successfully!

## Delets all files and folders in user's Local CrashDumps folder. 
Get-ChildItem "C:\users\*\AppData\Local\CrashDumps\*" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of Local CrashDumps have been removed successfully!

## Delets all old OST files in user's Local Microsoft Outlook folder. 
Get-ChildItem "C:\users\*\AppData\Local\Microsoft\Outlook\*.OST" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of old OST files on Local Microsoft Outlook have been removed successfully!

## Delets all files and folders in MS Teams Service Worker CacheStorage folder. 
Get-ChildItem "C:\users\*\AppData\Roaming\Microsoft\Teams\Service Worker\CacheStorage\*" -Recurse -Force -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The contents of MS Teams Service Worker CacheStorage have been removed successfully!
